...
```{r}
climate <- read.csv("/Users/johnbobo/analytics_edge/data/climate_change.csv")
```
...
```{r}
climate_lm <- lm(Temp ~ . -Year -Month, data=train)
summary(climate_lm)
```


**What is the model $R^2$ (the "Multiple R-squared" value)?**  
**Answer:** `r summary(climate_lm)$r.squared`
...
**Answer:** MEI, CO2, CFC.11, CFC.12, TSI, and Aerosols are all significant.

#### Problem 2.1 - Understanding the Model

Current scientific opinion is that nitrous oxide and CFC-11 are greenhouse gases: gases that are able to trap heat from the sun and contribute to the heating of the Earth. However, the regression coefficients of both the N2O and CFC-11 variables are **negative**, indicating that increasing atmospheric concentrations of either of these two compounds is associated with lower global temperatures.

**What is the simplest correct explanation for this contradiction?**  
**Answer:** Multicollinearity. N2O and CFC.11 are correlated with other variables in the data set. 
...
```{r}
climate_lm_2 <- lm(Temp ~ MEI+TSI+Aerosols+N2O, data=train)
summary(climate_lm_2)
```

**What is the coefficient of N2O in this reduced model?**  
**Answer:** `r coef(climate_lm_2)["N2O"]`

**What is the model $R^2$?**  
**Answer** `r summary(climate_lm_2)$r.squared`
...
```{r}
step_model <- step(climate_lm)
summary(step_model)
```


**What is the $R^2$ value of the model produced by the step function?**  
**Answer**: `r summary(step_model)$r.squared`
...
SSE <- sum((pred_test - test$Temp)^2)
SST <- sum((mean(train$Temp) - test$Temp)^2)
R2 <- 1 - SSE/SST
```

**Enter the testing set R2:** `r R2`